...
when_to_use: Use when the user asks about nbcli - how to use it, how to configure it, how to extend it, or how to modify it.
---
...
`zone: dev.example.com` is resolved to the matching zone ID automatically. For an A record:
...
```bash
nbcli filter record name=pg-dev zone=dev.example.com --ud 'value=db-new.example.com'
```
