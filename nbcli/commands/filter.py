"""Sub command to filter and act on search results of specified model."""
...
class Filter:
    """Filter search and act on results of model."""
    ...
    def delete(self, result):
        """Delete returned results of filter command."""
    ...
    def update(self, result, nba):
        """Update returned results of filter command with given update args."""
        assert is_list_of_records(result)
        anslist = list()
        for obj in result:
            anslist.append("{} ({})".format(str(obj), str(obj.id)))
    ...
    def detail(self, result, detail, *args, **kwargs):
        """Return results from detail endpoint."""
        assert is_list_of_records(result)
        self.result = list()
        for obj in result:
...
class FilterSubCommand(BaseSubCommand):
    ...
    def setup(self):
        """Add argaprse arguments to filter subcommand."""
        ...
        obj_meth.add_argument(
            "-c", "--count", action="store_true", help="Return the count of objects in filter."
        )

        obj_meth.add_argument(
            "-D", "--delete", action="store_true", help="Delete Object(s) returned by filter. [WIP]"
        )
